...
uniform vec2 uResolution, uPatternResolution, uMouse;
...
void main() {
  float px = 1.0 / min(uPatternResolution.x, uPatternResolution.y);
...
export default function MagicRings({
  ...
  const uniforms = {
      ...
      uPatternResolution: { value: new THREE.Vector2() },
    ...
    const resize = () => {
      ...
      const patternSize = Number.parseFloat(getComputedStyle(mount).getPropertyValue('--magic-rings-pattern-size')) || Math.min(w, h);
      renderer.setSize(w, h);
      renderer.setPixelRatio(dpr);
      uniforms.uResolution.value.set(w * dpr, h * dpr);
      uniforms.uPatternResolution.value.set(patternSize * dpr, patternSize * dpr);
    };
    resize();
